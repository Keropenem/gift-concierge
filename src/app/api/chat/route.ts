...
【重要な分類ルール】
1. save_sender_profile: 贈り手（ユーザー自身）の年齢・性別・職業・趣味。受け手の情報は含めない。
2. save_recipient_profile: 受け手（贈る相手）の情報。nicknameは必須。関係性もここ。
3. save_sender_memories: 贈り手のギフト選びに活かせる永続的な特徴のみ。
4. save_recipient_notes: 受け手に関する自由記述。性格・エピソード・関係性の詳細など。nicknameは必須。
5. save_proposal: 会話の中で具体的な商品名を提案した場合、必ず呼び出すこと。商品名・価格・URL・製造元・ストーリー等を保存する。

【save_proposalの判定基準】
会話の中に「〇〇を提案します」「おすすめは〇〇です」等、具体的な商品名が含まれていれば、それはsave_proposalの対象です。必ず呼び出してください。

【保存しないもの（save_proposal以外）】
- 予算、贈り物のきっかけ、季節情報、今回固有の条件

該当情報がない関数はスキップしてOK。ただしsave_proposalは商品提案があれば必ず呼び出すこと。`
        }],
      },
    ];
