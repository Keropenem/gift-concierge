...
async function extractProfileData(
  ...
  try {
    const extractionPrompt = [
      ...
      {
        role: "user" as const,
        parts: [{ text: "ここまでの会話から判明した贈り手と受取り手のプロフィール情報を save_profile_data で保存してください。" }],
      },
    ];
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        tools: [{ functionDeclarations: [extractionTool] }],
      },
    });

    const parts = response.candidates?.[0]?.content?.parts || [];
    for (const part of parts) {
      if (part.functionCall && part.functionCall.name === "save_profile_data") {
...
export async function POST(request: NextRequest) {
  ...
  const { message, profile, recipient, userId } = body;
  ...
  try {
    ...
    extractProfileData(session.history, session).catch(
